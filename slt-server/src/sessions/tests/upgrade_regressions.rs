...
#[tokio::test]
async fn session_allows_new_upgrade_id_after_uncommitted_switch() {
    let (join, mut client, tx, _tun_rx, mut udp_rx, limits, assigned, _registry) =
        spawn_session().await;

    let dcid = Cid::from([0xE9; MAX_DCID_LEN]);
    let scid = Cid::from([0xEA; MAX_DCID_LEN]);
    let register = make_register_payload(dcid, scid, CipherSuite::Aes128Gcm);
    let mut reg_buf = Vec::new();
    register.encode(&mut reg_buf).unwrap();
    write_tcp_message(&mut client, Message::RegisterCid { payload: &reg_buf }).await;
    ...
    let first_probe_nonce = 0xAAAA_0001u64;
    ...
    // Simulate lost `SwitchAck`: keep the server on TCP and start a fresh attempt.
    let second_upgrade_id = 0xD010;
    let second_probe_nonce = 0xBBBB_0002u64;
